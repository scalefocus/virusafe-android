...
import bg.government.virusafe.app.bluetooth.PeripheralIgnoredGracePeriodIfNoProteGoCharacteristicInMin
import bg.government.virusafe.app.bluetooth.PeripheralIgnoredTimeoutInSec
...
class ProteGoScanner(
	...
	override fun enable(inMode: ScannerInterface.Mode) {
		...
		val transformer: ObservableTransformer<ClassifiedPeripheral.ProteGo, BeaconIdRemote> = when (inMode) {
			...
			ScannerInterface.Mode.SCAN_AND_EXCHANGE_BEACON_IDS -> exchangeBeaconId(ProteGoConnector(beaconIdAgent, context))
		}
		val builder: ScanFilter.Builder = ScanFilter.Builder()
		builder.setServiceUuid(ParcelUuid.fromString(ProteGoServiceUUIDString))
		val filter: ScanFilter = builder.build()
		client.scanBleDevices(scanSettings, filter)
	...
	private fun scanOnlyBeaconId(): ObservableTransformer<ClassifiedPeripheral.ProteGo, BeaconIdRemote> =
		ObservableTransformer { proteGoScannedDevices ->
			proteGoScannedDevices.ofType(ClassifiedPeripheral.ProteGo.FullAdvertisement::class.java)
				...
				.map { BeaconIdRemote(it.beaconId, it.rssi, BeaconIdSource.SCANNER, it.bleDevice.macAddress) }
		}
